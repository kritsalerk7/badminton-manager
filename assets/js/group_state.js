...
// --- [PATCH] Compatibility helpers for groups.html ---

// รวม owner mappings + รายชื่อก๊วนทั้งหมดของผู้ใช้
export async function listMyGroupsWithOwned(){
  // สร้าง mapping ก๊วนที่เราเป็น owner -> users/{uid}/groups ก่อน
  await ensureOwnerMappings();
  // แล้วดึงรายการก๊วนจาก users/{uid}/groups (ทั้ง owner/member)
  return await listMyGroups();
}

// สร้างลิงก์เชิญรูปแบบ { gid, code } เพื่อให้ปุ่ม "คัดลอกลิงก์เชิญ" ใช้งานได้
export async function getInviteLink(groupId){
  const code = await createInvite(groupId);
  return { gid: groupId, code };
}

// alias ให้ตรงกับชื่อที่หน้า groups.html เรียก
export async function joinByCode(groupId, code){
  return await joinGroup(groupId, code);
}
