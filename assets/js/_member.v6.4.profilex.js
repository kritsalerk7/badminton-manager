  ...
  const items = dayList.map(x => {
    ...
    return `<li class="list-group-item day-row" data-id="${x.id}" draggable="true">
      <div class="row-grid right">
        ...
        <div class="actions-col d-flex gap-2 justify-content-end">
          <button type="button" class="btn btn-sm btn-outline-dark btn-profile" title="โปรไฟล์" draggable="false"><i class="bi bi-person-vcard"></i></button>
...
async function collectMonthsBack(centerISO, monthsBack){
  const ym0 = ymFromISO(centerISO);
  const yms = [];
  for (let i=0;i<monthsBack;i++){
    yms.push(addMonths(ym0, -i));
  }
  const results = [];
  for (const ym of yms){
    const days = await safeGetMonthDays(ym);
    for (const d of days){ if (!d.date) d.date = d.id; } // normalize
    results.push(...days);
  }
  // de-dup by date id just in case
  const map = new Map();
  for (const d of results){ map.set(d.id||d.date, d); }
  return Array.from(map.values()).sort((a,b)=> sortISO(b.date||b.id, a.date||a.id)); // desc
}
...
async function openProfileModal(memberId){
  try{
    ...
    // Monthly stats for currently selected month
    ...
    // Collect last 6 months for richer context
    const monthsBackData = await collectMonthsBack(state.date, 6);
    const recentRows = [];
    let s6 = 0, a6 = 0; // signed/arrived
    for (const d of monthsBackData){
      const p = extractPlayerFromDay(d, memberId);
      if (!p) continue;
      const dateISO = d.date || d.id;
      s6++; if (p.arrived) a6++;
      if (recentRows.length < 12){ recentRows.push({ date: dateISO, arrived: !!p.arrived }); }
    }
    const r6 = pct(a6, s6);
    const no6 = s6 - a6;
    // Fill overall 6 month stats
    ...
    if (rangeNote){
      const lastYM = addMonths(ymFromISO(state.date), -5);
      rangeNote.textContent = `ช่วง ${lastYM} ถึง ${ymFromISO(state.date)}`;
    }
    const recentEl = profileModalEl.querySelector("#pfRecent");
    if (recentEl){ recentEl.innerHTML = buildRecentList(recentRows); }
